  ...
  const messages: ChatCompletionMessageParam[] = [
    {
      role: "system",
      content:
        "You are an assistant that selects the most relevant person for a given question. Respond with only the person's ID (1 for Adam, 2 for Michał, or 3 for Jakub).",
    },
...
async function answerQuestion(
  ...
  const messages: ChatCompletionMessageParam[] = [
    {
      role: "system",
      content: `You are an assistant answering questions about ${
        person.name
      }. Use the following information: ${JSON.stringify(person)}`,
    },
    { role: "user", content: question },
  ];
